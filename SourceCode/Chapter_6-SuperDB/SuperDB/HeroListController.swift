...
class HeroListController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITabBarDelegate, NSFetchedResultsControllerDelegate{
    ...
    func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [AnyObject]? {
        ...
        var edit = UITableViewRowAction(style: .Normal, title: "Edit", handler: {
            action, indexPath in
            println(">>> Selected custom action for \(indexPath.row)")
        })
        
        var delete = UITableViewRowAction(style: .Default, title: "Delete", handler: {
            action, indexPath in
            //
        })
        return [ delete, edit ]
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "HeroDetailSegue"{
            if let _sender = sender as? NSManagedObject{
                ...
                detailController.hero = _sender //as NSManagedObject
            } else {
