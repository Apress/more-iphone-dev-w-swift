    ...
    var age:Int {
        get {
            if self.birthDate != NSNull() {
                ...
                var today = NSDate()
                var components = gregorian?.components(NSCalendarUnit.YearCalendarUnit, fromDate: self.birthDate, toDate: NSDate(), options: .allZeros)
    ...
    func validateBirthDate(ioValue: AutoreleasingUnsafeMutablePointer<AnyObject?>, error:NSErrorPointer) -> Bool {
        ...
        if date.compare(NSDate()) == .OrderedDescending {
            if error != nil {
                ...
                var outError = NSError(domain: kHeroValidationDomain, code: kHeroValidationBirthdateCode, userInfo: userInfo)
